"""E2E tests for the downloader module using VCR cassettes (max_entries=3 for speed)."""
...
e2e_vcr = vcr.VCR(
    ...
    before_record_response=lambda resp: {
        **resp,
        "body": {
            "string": resp["body"]["string"][:100] if resp.get("body", {}).get("string") else b""
        },
    },
)
...
class Test404Handled:
    """Uses responses mock — CDN never returns 404 for valid paths."""
...
class TestNetworkErrorHandled:
    def test_network_error_handled(self, download_dir: Path):
        ...
        ConnectionError cannot be recorded by VCR (socket-level failure).
        """
        import responses
...
class TestChecksumMismatch:
    def test_checksum_mismatch_backs_up_old_file(self, download_dir: Path):
        ...
        Requires fake content differing from real CDN — use responses.
        """
        import responses
...
class TestHTTP500:
    """Uses responses mock — CDN never returns 500 for valid paths."""
