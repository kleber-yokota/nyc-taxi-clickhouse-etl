"""Tests for _download_entry return values in downloader_download module."""
...
from extract.downloader.downloader import _download_entry
...
class TestDownloadEntryReturns:
    ...
    def test_download_entry_returns_failed_on_http_error(self, tmp_path: Path):
        ...
        with patch("extract.downloader.downloader_download._fetch_content", side_effect=http_error):
    ...
    def test_download_entry_returns_failed_on_network_error(self, tmp_path: Path):
        ...
        with patch("extract.downloader.downloader_download._fetch_content", side_effect=requests.RequestException("conn refused")):
            result = _download_entry(entry, tmp_path, state)
